...
export default {
  ...
  data() {
    return {
      materialArr: [
        {
          img: "https://static.theone.art/pc/active/linkage/linkage-img1.png",
          title: "5个国潮博物馆",
          uuid: null,
          value1: null,
          selectNum: 0,
  ...
  methods: {
    async init() {
      const result = await this.$api.$service.config_get({ key: "love512" });
      if (result.code == 200) {
        this.materialArr[0].value1 = result.data.confValue2;
        // this.materialArr[0].value1 = 2;
        this.materialArr[0].uuid = result.data.confValue1;
        this.materialArr[1].value1 = result.data.confValue4;
        // this.materialArr[1].value1 = 3;
        this.materialArr[1].uuid = result.data.confValue3;
      } else {
    ...
    async getNotice() {
      const result = await this.$api.$service.config_get({ key: "love512con" });
      if (result.code == 200) {
...
.ui-body {
  ...
  .icon {
    ...
    width: 0.64rem;
    height: 0.65rem;
    background: url("https://static.theone.art/pc/active/linkage/linkage-icon1.png");
    background-size: contain;
    @media screen and (max-width: 540px) {
      margin: 0.39rem auto;
    }
...
.banner {
  width: 100%;
  height: 350px;
  background: url("https://static.theone.art/pc/active/linkage/linkage-banner.png")
    no-repeat center;
  background-size: cover;
  background-position: 50%;
  @media screen and (max-width: 540px) {
    ...
    height: 3rem;
    box-sizing: border-box;
    background: url("https://static.theone.art/pc/active/linkage/linkage-bg-phone.png")
      no-repeat center;
    background-size: contain;
  }
}
