    ...
    <div class="dialog-content-wrap">
      ...
      <div class="dialog-content grayscale compose-page">
        ...
        <div v-else-if="listData.length > 0">
          <div class="treasure-list-wrap">
            ...
            <div class="list-content g-padding">
              <div
                ...
                :key="idx"
              >
                <div class="treasure-bar-content">
                  <div class="ui-icon icon-selected"></div>
                  <div
                    class="img-view"
          ...
          <div class="select-wrap" v-if="selectedSkuList.length > 0">
            <div class="s-title">
              您已选择的藏品
              <small
                >{{ selectedSkuList.length }}/{{
                  userSelectedRule.needNum
                }}</small
              >
              ：
            </div>
            <div class="list g-padding">
              <div
                class="select-bar"
          ...
          <div class="ui-btn-group g-padding">
            <div>
              <div class="buy-btn" @click="goMarket">去购买</div>
            </div>
            <div>
              <div class="compose-btn" @click="onSubmit">去合成</div>
            </div>
          </div>
        </div>
        <div v-else style="padding-top: 40px">
          <empty> </empty>
          <div class="buy-btn empty-buy-btn" @click="goMarket">去购买</div>
        </div>
      </div>
    </div>
...
export default {
    ...
    selectedSkuList(val) {
      const findSelected = (commodityId) => {
        for (let i = 0; i < val.length; i++) {
          if (
            val[i].commoditySku &&
            val[i].commoditySku.commodityId == commodityId
          ) {
            return true;
          }
        }
  ...
  methods: {
    async init(info) {
      ...
      this.selectedSkuList = info.selectList
        ? JSON.parse(JSON.stringify(info.selectList))
        : [];
    ...
    listenUserSelectedSku(commodity, selectSkuArr, $obj) {
      ...
      if (newList.length > this.userSelectedRule.needNum) {
        this.$message.warning(
          `您已选择的藏品数量超出合成所需${
            newList.length - this.userSelectedRule.needNum
          }个`
        );
      } else {
    ...
    async getList() {
      this.isLoading = true;
      const res = await this.$api.service.treasureSku_queryTreasureByCategory({
        ...this.listParams,
        categoryUuid: this.userSelectedRule.uuid,
      });
    ...
    onShowSku(info, idx) {
      if (this.userSelectedRule.needNum == this.selectedSkuList.length) {
        this.$message.warning("您已选择足够的藏品数量，请点击合成");
      } else {
        this.selectedIdx = idx;
        this.$refs.TreasureSkuListRef.init(info, this.selectedSkuList);
      }
    },
    onSubmit() {
      if (this.selectedSkuList.length > 0) {
        if (this.userSelectedRule.needNum == this.selectedSkuList.length) {
          this.$emit("listen", {
            selectedSkuList: this.selectedSkuList,
            type: this.userSelectedRule,
          });
          this.isShow = false;
        } else {
          this.$message.warning("您已选择的藏品不足进行合成，请核对藏品数量");
        }
      } else {
...
.icon-selected {
  background-image: url("//static.theone.art/pc/active/compose/selected-icon.png");
}
.fade-enter-active,
.fade-leave-active {
