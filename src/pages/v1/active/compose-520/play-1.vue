<template>
  <div>
    <div class="ui-body" v-if="isStart">
      ...
      <div class="material grayscale">
        ...
        <div class="editor-wrap">
          <div class="type-wrap">
            <div
              class="item-box"
              v-for="(item, idx) in materialArr[0].list"
              :key="idx"
            >
              <div
                class="img"
              ...
              ></div>
              <div class="item-text">
                <div class="item-tl color padding-top">{{ item.name }}</div>
              ...
              <div @click="goSelect(item,materialArr[0])" :class="['item-btn color']">
                {{ materialArr[0].btnText }}
              </div>
            </div>
          </div>
          <div class="type-wrap">
            <div class="item-box">
              <div
                class="img"
                :style="'background-image:url(' + materialArr[1].cover + ')'"
              ></div>
              <div class="item-text">
                <div class="item-tl color padding-top">
                  {{ materialArr[1].name }}
                </div>
                <div class="item-num padding-top">
                  需要：任意{{ materialArr[1].needNum }}个
                </div>
                <div class="color padding-top">
                  已选择：{{ materialArr[1].selectNum }}个
                </div>
              </div>
              <div
                @click="goSelect(materialArr[1])"
                :class="[
                  'item-btn color',
                  materialArr[1].selectNum < materialArr[1].needNum
                    ? ''
                    : 'item-btn-bg',
                ]"
              >
                {{ materialArr[1].btnText }}
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="linkage">
          ...
          <p class="linkage-tl item-tl color padding-top">
            {{ compoundInfo.name }}
          </p>
          <div
    ...
    <div v-else class="ui-empty">活动尚未开始</div>
      ...
      materialArr: [
        {
          ...
          selectList: [], //已经选择
          list: [
            { uuid: "djniccikjdak", name: "二次元动漫馆", cover: "" },
            { uuid: "dccwdcwercw", name: "国潮博物馆馆", cover: "" },
          ],
        },
        {
          uuid: "adcwccidcfckjdak",
          name: "平台限定空投",
          cover: "",
          needNum: 2,
          selectNum: 0,
  ...
  computed: {
    isCompound() {
      ...
      return this.materialArr.every((item) => {
        return item.needNum == item.selectNum;
      });
    },
    userInfo() {
  ...
  methods: {
    async init() {
      ...
      if (result.data.code == 200) {
        ...
        /*     if (data) {
          this.isStart = true;
          this.materialArr[0].value1 = data.confValue2;//所需数量
    ...
    goSelect(info,data) {
      if (this.userInfo.userUuid && this.userInfo.userUuid.length > 0) {
        ...
        if(data){
          _info={..._info,selectList:data.selectList}
        }
        this.$refs.TreasureListRef.init(_info);
      } else {
    ...
    listenUserSelectedSku(info) {
      ...
      if (info.type.uuid) {
        this.materialArr[1].selectList = info.selectedSkuList;
        this.materialArr[1].selectNum = info.selectedSkuList.length;
      } else {
        this.materialArr[0].selectList = this.$common.uniqueAry([...this.materialArr[0].selectList,...info.selectedSkuList],'id');
        this.materialArr[0].selectNum = this.materialArr[0].selectListlength;
        this.materialArr[0].list.forEach((item) => {
