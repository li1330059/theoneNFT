...
export default {
  ...
  methods: {
    ...
    listenUserSelectedSku(commodity, selectSkuArr, $obj) {
      ...
      if (newList.length > this.userSelectedRule.value1) {
        this.$message.warning(
          `您已选择的藏品数量超出合成所需${
            newList.length - this.userSelectedRule.value1
          }个`
        );
      } else {
    ...
    onShowSku(info, idx) {
      if (this.userSelectedRule.value1 == this.selectedSkuList.length) {
