...
function Browser() {
    ...
    if (weibo) browser.weibo = true, weibo.version = wechat[1]
    if (tws) {
      towords.istowords = true, towords.version = tws[1]
    } else if (osx && !safari) {
      // 拓词ios9.10版本额，兼容ipad下无法判断的问题
      towords.istowords = true, towords.version = '9.10'
    }
    if (huawei) browser.huawei = true;
    if (xiaomi) browser.xiaomi = true;
  
  
    os.tabvar = !!(ipad || playbook || (android && !ua.match(/Mobile/)) ||
      (firefox && ua.match(/Tablet/)) || (ie && !ua.match(/Phone/) && ua.match(/Touch/)))
    os.phone = !!(!os.tabvar && !os.ipod && (android || iphone || webos || blackberry || bb10 ||
      (chrome && ua.match(/Android/)) || (chrome && ua.match(/CriOS\/([\d.]+)/)) ||
      (firefox && ua.match(/Mobile/)) || (ie && ua.match(/Touch/))))
  
    this.os = os;
    this.browser = browser;
    this.thirdParty = mqqbrowser || qq || wechat || weibo;
